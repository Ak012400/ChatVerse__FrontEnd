...
import {
  Mic, MicOff, Hand, Send, Shield, Crown, X, Lock, Globe,
...
// ============================================================
//  RoastRoomPage — comedic 5v5 mutual roast.
//
//  Architecture: shares the StageBracket backend with DebateV2.
//  `mode: 'roast'` on the config; sides are "ROASTERS" (left) vs
//  "ROASTEES" (right) instead of Pro/Con. Server requires a
//  host-provided topic — no public topic bank for roast rooms.
//
//  Visual identity: savage red → purple palette, skull glyph.
// ============================================================
...
export default function RoastRoomPage({ room, iAmHost, onLeave, onEndRoom }: Props) {
  ...
  return (
    <div className="cv-roast-stage cv-pop p-4 sm:p-5 border border-[var(--color-line)] relative">
      ...
      {/* Roast subject banner */}
      {round?.topic ? (
        <div className="cv-roast-topic mb-4">
          <div className="text-[10px] uppercase tracking-wider text-[var(--color-fg-faint)] mb-1 inline-flex items-center gap-1">
            <Flame size={10} className="text-red-400" /> Roast subject
          </div>
          <div className="text-sm font-medium text-[var(--color-fg)]">{round.topic}</div>
        </div>
      ) : iAmHost ? (
        <div className="cv-roast-topic mb-4 border border-amber-500/40 bg-amber-500/5">
          <div className="text-[10px] uppercase tracking-wider text-amber-300 mb-1">Heads-up</div>
          <div className="text-xs text-[var(--color-fg-dim)]">Roast rooms always need a host-provided subject. Set one in Config below before starting a round.</div>
        </div>
      ) : null}
      ...
      <div className="mt-4 flex items-center justify-between gap-2">
        <button onClick={onLeave} className="text-xs text-[var(--color-fg-mute)] hover:text-[var(--color-fg)] transition-colors">
          ← Back to Mehfil
        </button>
        <div className="text-[10px] text-[var(--color-fg-faint)] inline-flex items-center gap-1.5">
          <Skull size={10} /> {config?.secondsPerTurn ?? 90}s per roast · audience can raise hand to step in
        </div>
      </div>
    </div>
  )
...
function InviteCodeGate({
  ...
  return (
    <div className="cv-roast-stage cv-pop p-6 sm:p-8 border border-[var(--color-line)] max-w-md mx-auto">
      <div className="text-center space-y-4">
        ...
        <div>
          <h2 className="text-lg font-semibold cv-text-gradient">Private Roast</h2>
        ...
        <input
          ...
          className="w-full text-center font-mono tracking-widest text-lg px-4 py-2 rounded-md bg-[var(--color-surface-2)] border border-[var(--color-line)] focus:border-red-400 outline-none"
          autoFocus
        />
...
function RoundStatusPill({ round }: { round: any | null }) {
  ...
  if (round.status === 'live') {
    return (
      <span className="cv-roast-pulse inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-full bg-red-500/15 text-red-300 border border-red-500/40">
...
function HostControlsBar({
  ...
  return (
    <div className="cv-roast-monitor-rail rounded-md border border-[var(--color-line)] bg-[var(--color-surface-1)] p-3 flex flex-wrap items-center gap-2">
      ...
      {showConfig && config && (
        <div className="basis-full mt-2 grid grid-cols-1 sm:grid-cols-4 gap-2 text-[11px]">
          ...
          <div className="sm:col-span-4">
            <div className="text-[10px] uppercase tracking-wider text-[var(--color-fg-faint)] mb-1 inline-flex items-center gap-1">
              <Flame size={9} className="text-red-400" /> Roast subject (required)
            </div>
            <div className="flex gap-1.5">
              <input
                value={topicDraft}
                onChange={(e) => setTopicDraft(e.target.value.slice(0, 200))}
                placeholder="Who or what gets roasted today?"
                className="flex-1 px-3 py-1.5 rounded-md bg-[var(--color-surface-2)] border border-[var(--color-line)] focus:border-red-400 outline-none text-sm"
              />
              <Button
        ...
        {(['public', 'private'] as StageBracketPrivacy[]).map((p) => (
          <button
            ...
            className={[
              ...
              value === p ? 'bg-red-500/20 text-red-300' : 'bg-[var(--color-surface-2)] text-[var(--color-fg-dim)] hover:text-[var(--color-fg)]',
            ].join(' ')}
          >
        ...
        {options.map((n) => (
          <button
            ...
            className={[
              ...
              value === n ? 'bg-red-500/20 text-red-300' : 'bg-[var(--color-surface-2)] text-[var(--color-fg-dim)] hover:text-[var(--color-fg)]',
            ].join(' ')}
          >{n}</button>
        ))}
      </div>
...
function SideStage({
  ...
  const myLabel = label === 'ROASTERS' ? 'left' : 'right'
  return (
...
function SeatTile({
  ...
  return (
    <div
      className={[
        'cv-roast-seat relative rounded-md border px-2.5 h-10 flex items-center gap-2 transition-colors',
        empty
          ? 'border-dashed border-[var(--color-line)] bg-[var(--color-surface-2)] text-[var(--color-fg-mute)]'
          : 'bg-[var(--color-surface-2)] text-[var(--color-fg)]',
        isActive ? 'cv-roast-seat-active' : '',
      ].join(' ')}
    ...
    >
      ...
      {isActive && <Mic size={12} className="text-red-300" />}
...
function CenterColumn({
  ...
  return (
    <div className="hidden lg:flex flex-col items-center justify-start px-2 text-[var(--color-fg-faint)] gap-2">
      ...
      {amICurrentSpeaker && (
        <div className="cv-roast-pulse mt-2 inline-flex items-center gap-1 text-[10px] uppercase tracking-wider font-bold text-red-300 bg-red-500/15 border border-red-500/40 px-2 py-1 rounded-full">
    ...
    <div className="rounded-md border border-[var(--color-line)] bg-[var(--color-surface-1)] flex flex-col min-h-[360px] max-h-[60vh]">
          ...
          return (
            <div key={m.id} className={`group flex gap-1.5 ${mine ? 'flex-row-reverse' : ''}`}>
              <div className={`max-w-[80%] rounded-2xl px-3 py-1.5 text-sm ${
                m.senderIsHost
                  ? 'bg-red-500/20 border border-red-400 text-[var(--color-fg)]'
                  : mine
      ...
      <div className="border-t border-[var(--color-line)] p-2 flex items-end gap-1.5">
        <textarea
          ...
          rows={2}
          maxLength={1000}
          placeholder="Throw a line…"
          className="flex-1 resize-none rounded-md cv-glass text-sm leading-relaxed p-2 border border-[var(--color-line)] focus:outline-none focus:border-red-400"
          onKeyDown={(e) => {
      ...
      <div className="cv-sheet-up w-full sm:max-w-md sm:rounded-xl rounded-t-2xl bg-[var(--color-surface-1)] border border-[var(--color-line)] shadow-2xl">
        <header className="p-4 border-b border-[var(--color-line)] flex items-center justify-between">
          <div className="text-sm font-semibold inline-flex items-center gap-2">
            <Skull size={14} className="text-red-300" /> Take a roast seat
          </div>
          <button onClick={onClose}><X size={16} /></button>
        </header>
        <div className="p-4 space-y-3 text-sm">
          <p className="text-[11px] text-[var(--color-fg-mute)] leading-snug">
            Pick whether you want to <strong>roast</strong> the others or be the one <strong>roasted</strong>.
            Public rooms seat you automatically.
          </p>
          <div>
            <label className="block text-[11px] uppercase tracking-wider font-medium text-[var(--color-fg-faint)] mb-1">Side</label>
            <div className="inline-flex rounded-md overflow-hidden border border-[var(--color-line)]">
              {(['left', 'right', 'either'] as StageBracketPreferredSide[]).map((s) => (
                <button
                  ...
                  className={[
                    ...
                    side === s ? 'bg-red-500/20 text-red-300' : 'bg-[var(--color-surface-2)] text-[var(--color-fg-dim)] hover:text-[var(--color-fg)]',
                  ].join(' ')}
                >
                  {s === 'left' ? 'Roaster' : s === 'right' ? 'Roastee' : 'Either'}
                </button>
              ))}
            </div>
          </div>
          <div>
            <label className="block text-[11px] uppercase tracking-wider font-medium text-[var(--color-fg-faint)] mb-1">Note (host-only)</label>
            <input
              value={note}
              onChange={(e) => setNote(e.target.value.slice(0, 120))}
              placeholder="Your angle / persona"
              className="w-full px-3 py-2 rounded-md bg-[var(--color-surface-2)] border border-[var(--color-line)] focus:border-red-400 outline-none text-sm"
            />
          </div>
        </div>
        <footer className="p-4 border-t border-[var(--color-line)]">
          <Button fullWidth variant="primary" onClick={() => onSubmit(side, note.trim())} leftIcon={<Skull size={14} />}>
            Raise hand
          </Button>
        </footer>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/55 backdrop-blur-sm">
      <div className="cv-sheet-up w-full sm:max-w-md sm:rounded-xl rounded-t-2xl bg-[var(--color-surface-1)] border border-[var(--color-line)] shadow-2xl">
        <header className="p-4 border-b border-[var(--color-line)] flex items-center justify-between">
          <div className="text-sm font-semibold inline-flex items-center gap-2">
            ⚡ Cut in on the current speaker
          </div>
          <button onClick={onClose}><X size={16} /></button>
        </header>
        <div className="p-4 space-y-3 text-sm">
          <p className="text-[11px] text-[var(--color-fg-mute)] leading-snug">
            If the host approves, the active speaker's mic mutes briefly and you take the floor.
          </p>
          <input
            value={note}
            onChange={(e) => setNote(e.target.value.slice(0, 120))}
            placeholder="Got a one-liner?"
        ...
        {seatNoms.map((n) => (
          <div key={n.id} className="mt-1.5 rounded-md border border-[var(--color-line)] bg-[var(--color-surface-2)] p-2 text-xs space-y-1">
            <div className="font-medium inline-flex items-center gap-1.5">
              {n.username}
              <span className="text-[10px] text-[var(--color-fg-mute)]">· prefers {n.preferredSide === 'left' ? 'roaster' : n.preferredSide === 'right' ? 'roastee' : 'either'}</span>
            </div>
              ...
              <div className="flex flex-col gap-1 pt-1">
                {emptyLeft.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    <span className="text-[10px] uppercase text-red-300">Roasters</span>
                    {emptyLeft.map((s) => (
                      <button
                        key={s.id}
                ...
                {emptyRight.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    <span className="text-[10px] uppercase text-purple-300">Roastees</span>
                    {emptyRight.map((s) => (
                      <button
                        key={s.id}
                        onClick={() => { onAssign(n.id, 'right', s.position); setPicking(null) }}
                        className="text-[10px] h-6 px-2 rounded-full bg-purple-500/15 text-purple-300 border border-purple-500/40 hover:bg-purple-500/30"
                      >Seat {s.position + 1}</button>
                    ))}
                  </div>
                )}
                <button onClick={() => setPicking(null)} className="text-[10px] text-[var(--color-fg-mute)] hover:text-[var(--color-fg)] self-start">Cancel</button>
              </div>
            ) : (
              <div className="flex flex-wrap gap-1 pt-1">
                <button
                  onClick={() => setPicking(n.id)}
                  className="text-[10px] h-6 px-2 rounded-full bg-red-500/20 text-red-300 border border-red-400 inline-flex items-center gap-1"
      ...
      {challengeNoms.length > 0 && (
        <div>
          <div className="text-[10px] uppercase tracking-wider font-semibold text-amber-300 inline-flex items-center gap-1.5">
            ⚡ Cut-in requests ({challengeNoms.length})
          </div>
          {challengeNoms.map((n) => (
...
void Copy; void Check; void Crown
